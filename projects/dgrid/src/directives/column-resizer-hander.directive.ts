...
@Directive({
    selector: '[columnResizerHandler]'
})
export class ColumnResizerHanderDirective implements OnDestroy {

    @Input('columnResizerHandler')
    public cellWrapper: Element;
    @Input()
    ...
    @HostListener('mousedown', ['$event']) public onMouseDown(evt: any): void {
        ...
        if (!this.columnNodeEl) {
            this.columnNodeEl = this.cellWrapper.parentElement.parentElement;
            this.tableNodeEl = this.columnNodeEl.parentElement.parentElement;
        }
        // 表格禁用文本选择
        ...
        const thNodeClientRect: DOMRect = this.columnNodeEl.getBoundingClientRect();
        ...
        const resizeEnd: (e: any) => void = e => {
            ...
            if (this.columnType === 'unfrozen') {
                ...
                this.renderer2.setStyle(this.columnNodeEl, 'width', `${this.size}px`);
            }
            // this.renderer2.setStyle(this.columnNodeEl, 'width', `${this.size}px`);
            this.renderer2.setAttribute(this.columnNodeEl, 'sign-width', `${this.size}`);
